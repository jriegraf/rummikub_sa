...
class SwingGui(co: ControllerInterface, var game: Game) extends MainFrame {
  ...
  val finishButton: Button = new Button("Finish") {
    enabled = false
  }
  ...
  val field: SwingGrid = new SwingGrid(8, 13) {
    border = Swing.EmptyBorder(0, 10, 10, 10)
  }
  val rack: SwingGrid = new SwingGrid(4, 13) {
    border = Swing.EmptyBorder(0, 10, 10, 10)
  }
  ...
  val center: BoxPanel = new BoxPanel(Orientation.Vertical) {
    contents += field
    contents += rack
